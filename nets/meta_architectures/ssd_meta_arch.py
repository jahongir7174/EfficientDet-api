...
from nets.core import box_list
from nets.core import box_list_ops
from nets.core import matcher
from nets.core import model
...
from nets.utils import variables_helper
...
class SSDMetaArch(model.DetectionModel):
    def __init__(self,
        ...
        Args:
          ...
          score_conversion_fn: callable elementwise nonlinearity (that takes tensors
            ...
            to probabilities.
          classification_loss: an nets.core.losses.Loss object.
          localization_loss: a nets.core.losses.Loss object.
          ...
          expected_loss_weights_fn: If not None, use to calculate
            ...
            third_party/tensorflow_models/nets/utils/ops.py as examples.
    ...
    def preprocess(self, inputs):
        ...
        with tf.name_scope('Preprocessor'):
            ...
            return shape_utils.resize_images_and_return_shapes(normalized_inputs, self._image_resizer_fn)
    ...
    def _compute_clip_window(preprocessed_images, true_image_shapes):
        ...
        return tf.stack([tf.zeros_like(true_heights),
                         tf.zeros_like(true_widths), true_heights / padded_height,
                         true_widths / padded_width], axis=1)

    def predict(self, preprocessed_inputs, true_image_shapes, **kwargs):
        ...
        if self._box_predictor.is_keras_model:
            ...
            with slim.arg_scope([slim.batch_norm],
                                is_training=(self._is_training and not self._freeze_batchnorm),
                                updates_collections=batchnorm_updates_collections):
                predictor_results_dict = self._box_predictor.predict(feature_maps,
                                                                     self._anchor_generator.num_anchors_per_location())
        predictions_dict = {'preprocessed_inputs': preprocessed_inputs,
        ...
        if self._return_raw_detections_during_predict:
            predictions_dict.update(self._raw_detections_and_feature_map_inds(predictions_dict['box_encodings'],
                                                                              boxlist_list))
    ...
    def _get_feature_map_spatial_dims(feature_maps):
        ...
        """
        fm_shapes = [shape_utils.combined_static_and_dynamic_shape(feature_map) for feature_map in feature_maps]
        return [(shape[1], shape[2]) for shape in fm_shapes]
    ...
    def _assign_targets(self,
        ...
        if train_using_confidences:
            return target_assigner.batch_assign_confidences(self._target_assigner,
                                                            self.anchors,
                                                            groundtruth_boxlists,
                                                            groundtruth_confidences_with_background_list,
