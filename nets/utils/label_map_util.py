...
from nets.protos import string_int_label_map_pb2
...
def convert_label_map_to_categories(label_map, max_num_classes, use_display_name=True):
    ...
    if not label_map:
        label_id_offset = 1
        for class_id in moves.range(max_num_classes):
...
def get_label_map_dict(label_map_path_or_proto, use_display_name=False, fill_in_gaps_and_background=False):
    ...
    if fill_in_gaps_and_background:
        ...
        if len(values) != max(values) + 1:
            for value in moves.range(1, max(values)):
...
def get_label_map_hierarchy_lut(label_map_path_or_proto, include_identity=False):
    ...
    hierarchy_dict = {'ancestors': collections.defaultdict(list),
